...
| 오늘의 한마디 → 유명인 명언 700개 교체 | 🟡 진행 중 (145/700) | v0.0.22~ | Stage B 완료 후 착수. 철학자/사상가/작가/과학자/투자자 실제 검증 어록, `{text,author,role}` 구조, 12시간(KST) 단위 순환. 20개(v0.0.22)→56개(v0.0.23)→116개(v0.0.24)→145개(v0.0.25). 섹션명 "오늘의 폼 미친 한마디"→"오늘의 인생 한마디"로 변경. 인물 풀이 줄어들며 배치 #3부터 검증된 인물의 2번째 어록도 활용(1인당 최대 2개). 출처 불확실한 오귀속 명언은 계속 확인 후 배제 |
...
window.AppData = {
  quotes: [...],          // 오늘의 인생 한마디: { text, author, role } 객체 배열 (v0.0.22~, 700개 목표로 단계적 확장 중)
