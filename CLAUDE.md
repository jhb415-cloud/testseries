...
| 오늘의 한마디 → 유명인 명언 700개 교체 | 🟡 진행 중 (116/700) | v0.0.22~ | Stage B 완료 후 착수. 철학자/사상가/작가/과학자/투자자 실제 검증 어록, `{text,author,role}` 구조, 12시간(KST) 단위 순환. 1단계 샘플 20개(v0.0.22)→2단계 배치 #1 36개(v0.0.23)→배치 #2 60개(v0.0.24)로 116개 확보. 섹션명 "오늘의 폼 미친 한마디"→"오늘의 인생 한마디"로 변경. 출처 불확실한 인터넷 밈 명언(아인슈타인/다윈/간디/괴테/갈릴레오/트웨인/에머슨/조지엘리엇/터닝/볼테르 등의 오귀속 명언)은 의도적으로 배제 |
