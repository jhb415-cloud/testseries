# 심리테스트 엔진 (test-engine) — STEP 2까지 진행된 프로토타입

## 버전 표기 규칙
이 하위 프로젝트는 메인 사이트의 `vX.Y.Z` 버전과 무관하게 **`test-engine vN (STEP N)`** 자체 표기를 씀 (engine.js/engine.css/themes/*.css 등 수정 파일 상단 주석 참고). 현재 `v2 (STEP 2)`.

## 개요
이 폴더는 기존 사이트(`index.html` / `app.js` / `data.js`)와 **완전히 분리된** 신규 프로토타입입니다.
...
- **결과**: 결과 이미지 + 타이틀 + 서브카피 + traits 리스트 + tip + (있으면) 관련 테스트 배너 + "다시하기"(인트로로)/"공유하기"(Web Share API, 미지원 시 클립보드 복사 폴백)/"🖼️ 이미지 저장"(Canvas로 세로형 카드 합성 → 모달에 미리보기+"길게 눌러 저장" 안내+다운로드 버튼)
