...
3. **2단계 워크플로우(2026-07-10~, 기본값)**: OpenAI(`gpt-image-1`) 단독 생성 품질이 부족하다고
   판단해 아래 방식으로 전환함 — 이 문서의 "샘플 우선 승인" 원칙 자체는 동일하게 유지된다.
   - **1단계(스타일 탐색, 수동)**: 승인된 컨셉의 포터블 프롬프트를 사용자에게 그대로 전달 →
     사용자가 Midjourney 등 외부 툴로 직접 샘플을 뽑아보고 마음에 드는 레퍼런스 이미지를 확정해
     전달한다(공식 API가 없어 Claude가 자동 생성하지 않음).
   - **2단계(프로덕션 자동화)**: 확정된 레퍼런스 이미지를 `tests/{testId}/assets/reference/`에
     넣고 `generate-assets-gemini.js`(Nano Banana Pro/`gemini-3-pro-image-preview`, 참조 이미지
     기반 스타일 고정)로 나머지 결과 세트를 일괄 생성한다. 상세는
     `test-engine/README.md`의 "이미지 생성 2단계 워크플로우" 절 참고.
   - 레퍼런스 이미지 없이 빠르게 텍스트 프롬프트만으로 뽑아야 하는 경우엔 기존
     `generate-assets.js`(OpenAI)를 계속 사용해도 된다 — 두 스크립트 다 살아있음.
